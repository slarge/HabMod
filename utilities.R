...
vsurf_bb <- function(file_x,
  ...
  sentfile <- paste("VSURF_req_", name , "_", Sys.time(),".xml",sep="")
  sentfile <- gsub(":", "_", sentfile)
  sentfile <- gsub(" ", "_", sentfile)
  
  #LOAD THE TEMPLATE#    
  templateFile = "vsurfTemplate.xml"
...
sac_maker <- function(svspp) {
  wf_dat <- all_dat %>%
    filter(SVSPP == svspp) 
  
  name <- gsub(" ", "_", stock_list$COMNAME[stock_list$SVSPP == svspp])
  ...
  wf_lt$SAC.pa <- as.vector(unname(SAC.pa)) 
  wf_lt$SAC.az <- as.vector(unname(SAC.az)) 
  wf_lt$SAC.bm <- as.vector(unname(SAC.bm)) 
